...
public class TableGenerator extends CustomComponent {
    ...
    private IndexedContainer patientListData = createDummyData();
    ...
    private void initLayout() {
        ...
        setMainWindow(new Window("Patient List", splitPanel));
    ...
    private void initAddressList() {
        contactList.setContainerDataSource(patientListData);
    ...
    private void initFilteringControls() {
        for (final String pn : visibleCols) {
            ...
            sf.addListener(new Property.ValueChangeListener() {
                public void valueChange(ValueChangeEvent event) {
                	patientListData.removeContainerFilters(pn);
                    if (sf.toString().length() > 0 && !pn.equals(sf.toString())) {
                    	patientListData.addContainerFilter(pn, sf.toString(),
                                true, false);
                    }
                    getMainWindow().showNotification(
                            "" + patientListData.size() + " matches found");
                }
            });
        }
    }
